export const paragraphStyles = {
    ...
    paragraphStyle: (isEditing: boolean, characterCount: number, isQuote: boolean, isTextCenter: boolean) => `${isEditing ? 'rounded':( characterCount === 0 ? 'bg-red-50' : '')} ${isQuote ? 'pl-[4rem] italic text-gray-600' : 'border-b-2 border-slate-300 border-dotted'} pr-2 pb-4 cursor-text min-h-[1.5rem] outline-none ${isTextCenter ? 'text-center' : 'text-justify'}`,
