...
export function useNavigation() {
    ...
    /**
      ...
      const navigateToAdjacentParagraph = useCallback((
        ...
        if(paragraphIndex <= 0 && direction === 'previous') return;
        if(paragraphIndex >= paragraphs.length - 1 && direction === 'next') return;
        
        let newParagraphIndex = paragraphIndex;
    
        if( direction === 'previous' ) newParagraphIndex--;
        else if (direction === 'next') newParagraphIndex++;
